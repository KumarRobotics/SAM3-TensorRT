...
void Sam3Processor::allocateBuffers()
{
    ...
    cudaMallocHost(&h_input_ids_, txt_len * sizeof(int32_t));
    cudaMallocHost(&h_attention_mask_, txt_len * sizeof(int32_t));

    cudaMalloc(&d_input_ids_, txt_len * sizeof(int32_t));
    cudaMalloc(&d_attention_mask_, txt_len * sizeof(int32_t));
}
...
void Sam3Processor::uploadText(const std::vector<int32_t>& ids, const std::vector<int32_t>& mask) 
{
    const int len = params_.text_padding;

    std::memcpy(h_input_ids_,      ids.data(),  len * sizeof(int32_t));
    std::memcpy(h_attention_mask_, mask.data(), len * sizeof(int32_t));

    cudaMemcpyAsync(d_input_ids_, h_input_ids_, len * sizeof(int32_t), cudaMemcpyHostToDevice, stream_);
    cudaMemcpyAsync(d_attention_mask_, h_attention_mask_, len * sizeof(int32_t), cudaMemcpyHostToDevice, stream_);
}
...
Sam3TextInput Sam3Processor::preprocessText(const std::string& text) 
{
    ...
    const int len = params_.text_padding;  // 32

    std::vector<int> raw = tokenizer_.tokenize(text);

    const int real_len = std::min(static_cast<int>(raw.size()), len);

    std::vector<int32_t> ids (len, 0);   // zero-pad
    std::vector<int32_t> mask(len, 0);

    for (int i = 0; i < real_len; ++i) {
        ids[i] = static_cast<int32_t>(raw[i]);
        mask[i] = 1;
    }
